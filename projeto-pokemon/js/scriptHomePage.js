...
function escolher(ID) {
  ...
  if (ID == 1) {
    ...
    imagem.setAttribute(
      "src",
      `../pictures/pokemonSprites/${gerarPokemon(1)}.png`
    );
  } else if (ID == 2) {
    imagens.style.background = "#f09537";
    imagem.setAttribute(
      "src",
      `../pictures/pokemonSprites/${gerarPokemon(4)}.png`
    );
  } else if (ID == 3) {
    imagens.style.background = "#4DB1E3";
    imagem.setAttribute(
      "src",
      `../pictures/pokemonSprites/${gerarPokemon(7)}.png`
    );
  } else {
    imagem.setAttribute(
      "src",
      `../pictures/pokemonSprites/${gerarPokemon(25)}.png`
    );
  }
...
function pressionarTecla(event) {
  ...
  if (tecla == "y") {
    ...
    location.replace("../statusPage/statusPage.html");
    return "y";
  } else if (tecla == "n") {
