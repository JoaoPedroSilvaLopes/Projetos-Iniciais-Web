...
function gerarPagina( {numeroPokedex, especie, simbolos, sprites, tipo, descricao} ) {
    ...
    window.paginaSobre = function(id) {
        ...
        conteudoInterno.innerHTML = `<p class='descricaopokedex' id='descricaopokedex${numeroPokedex}' style="background-color: ${gerarCores(tipo[0])}">${descricao}</p>
        <div class='divtipos' id='divtipos${numeroPokedex}'>
            <div class='textotipos' id='textotipos' style="background-color: ${gerarCores(tipo[0])}">
                <img ${simbolos[0]} width='20%' height='auto'></img>
                <strong>${gerarStringTipo(tipo[0])}</strong>
            </div>
            <div class='textotipos' id='textotipos' style="background-color: ${gerarCores(tipo[1])}">
                <img ${simbolos[1]} width='20%' height='auto'></img>
                <strong>${gerarStringTipo(tipo[1])}</strong>
            </div>
        </div>
        <p class="paragrafoDadosPokedex" style="color: ${gerarCores(tipo[0])}">Dados Pokédex</p>
    ...
    window.paginaStats = function(id) {
        ...
        conteudoInterno.innerHTML = `<p class='paragrafotitulostats' id='paragrafotitulostats' style="color: ${gerarCores(tipo[0])}">Base stats</p>
    ...
    window.paginaEvolucao = function(id) {
        ...
        if (evolucoes === undefined) {
            conteudoInterno.innerHTML = `<p class='paragrafotitulostats' id='paragrafotitulostats' style="color: ${gerarCores(tipo[0])}">Evoluções</p>
            <div class='evolucao'>
                <div class="evolucoes">
                    <div class="dadosEvolucao">
                        <img src=${pokedex[index * 1].sprites[0]} width='30%' height='auto'/>
                        <p class="descricaoEvolucao" style="color: ${gerarCores(tipo[0])}">Não possui evolução</p>
                    </div>  
                </div>
            </div>`
        }
            ...
            <div class='evolucao'>
                <div class="evolucoes">
                    <img src=${pokedex[evolucoes[0][0]].sprites[0]} width='30%' height='auto'/>
                    <div class="dadosEvolucao">
                        <img src=${[evolucoes[2][0]]} width='30%' height='auto'/>
                        <p class="descricaoEvolucao" style="color: ${gerarCores(tipo[0])}">${evolucoes[1][0]}</p>
                    </div>
                    <img src=${pokedex[evolucoes[0][1]].sprites[0]} width='30%' height='auto'/>
                </div>
                <div class="evolucoes">
                    <img src=${pokedex[evolucoes[0][0]].sprites[0]} width='30%' height='auto'/>
                    <div class="dadosEvolucao">
                        <img src=${[evolucoes[2][1]]} width='30%' height='auto'/>
                        <p class="descricaoEvolucao" style="color: ${gerarCores(tipo[0])}">${evolucoes[1][1]}</p>
                    </div>
                    <img src=${pokedex[evolucoes[0][2]].sprites[0]} width='30%' height='auto'/>
                </div>
                <div class="evolucoes">
                    <img src=${pokedex[evolucoes[0][0]].sprites[0]} width='30%' height='auto'/>
                    <div class="dadosEvolucao">
                        <img src=${[evolucoes[2][2]]} width='30%' height='auto'/>
                        <p class="descricaoEvolucao" style="color: ${gerarCores(tipo[0])}">${evolucoes[1][2]}</p>
                    </div>
                    <img src=${pokedex[evolucoes[0][3]].sprites[0]} width='30%' height='auto'/>
                </div>
            </div>`
        }
    ...
    window.mudarCor = function(id, chave, indexBotao) {
        ...
        if (chave === 0) {
            const index = id.slice(id.length - 3)
            const botao = document.getElementById(`botaonavegacao${indexBotao}${index}`)
    ...
    return `<div class='paginaPokedex' id='paginaPokedex${numeroPokedex}'>
        <div class='fotopokemon' id='fotopokemon${numeroPokedex}' style="background-color: ${gerarCores(tipo[0])}">
        ...
        <div class='conteudo' id='conteudo${numeroPokedex}'>
            ...
            <div class='conteudoInterno' id='conteudoInterno${numeroPokedex}'>
                <p class='descricaopokedex' id='descricaopokedex${numeroPokedex}' style="background-color: ${gerarCores(tipo[0])}">${descricao}</p>
                <div class='divtipos' id='divtipos${numeroPokedex}'>
                    <div class='textotipos' id='textotipos' style="background-color: ${gerarCores(tipo[0])}">
                        <img ${simbolos[0]} width='20%' height='auto'></img>
                        <strong>${gerarStringTipo(tipo[0])}</strong>
